...
Includes both standard and bitrate-neutral variants for comparison.
'''
...
# -----------------------------
def dct_2d(block):
    return cv.dct(block.astype(np.float32))  # convert spatial → frequency domain

# -----------------------------
# IDCT (Inverse DCT)
# -----------------------------
def idct_2d(block):
    return cv.idct(block.astype(np.float32))  # reconstruct spatial block
...
def get_quantization_matrix(quality):
    ...
    # Standard JPEG Q matrix (quality = 50)
    Q50 = np.array([
    ...
    # Scale matrix based on quality factor
    if quality < 50:
        scale = 5000 / quality
    else:
        scale = 200 - 2 * quality
...
# -----------------------------
# Bit Estimation (approx JPEG entropy coding)
# -----------------------------
...
def process_image(image, quality, importance_map, alpha=1.0):
    ...
    # Convert to YCbCr (better perceptual compression)
    ...
    for c in range(3):
        ...
        for i in range(0, padded.shape[0], block_size):
            for j in range(0, padded.shape[1], block_size):
                ...
                # Less aggressive adaptation at high quality
                ...
                if is_luma:
                    ...
                    # Chroma gets coarser compression
                    Q = Q_base * 2.0
...
def process_image_standard(image, quality):
    ...
    channels = 1 if len(padded_image.shape) == 2 else padded_image.shape[2]
...
# -----------------------------
# Bitrate-Neutral Adaptive Compression
# -----------------------------
def process_image_bitrate_neutral(image, quality, importance_map, alpha=1.0):
    """
    Redistributes bits without increasing total bitrate.
    Important regions → better quality
    Unimportant regions → more compression
    """
    block_size = 8
    ...
    # Compute scaling (normalized to keep bitrate same)
    raw_scales = 1.0 + alpha * (1.0 - scores) ** 2
    raw_scales = raw_scales / raw_scales.mean()
    raw_scales = raw_scales * 1.05
